...
pub async fn create_document(
    ...
    blocking(move || document::create_document(&db, title, parent_id, after_id, editor_id)).await
}
...
pub async fn list_documents(
    ...
    blocking(move || document::list_root_documents(&db)).await
}
...
pub async fn get_document_children(
    ...
    blocking(move || document::get_document_children(&db, &req.id)).await
}
...
pub async fn move_document_tree(
    ...
    blocking(move || document::move_document_tree(&db, req)).await
}
...
pub async fn get_breadcrumb(
    ...
    blocking(move || document::get_breadcrumb(&db, &req.id)).await
}
...
pub async fn import_text(
    ...
    blocking(move || document::import_text(&db, req)).await
}
...
pub async fn export_text(
    ...
    blocking(move || document::export_text(&db, &req.id, &req.format)).await
}
