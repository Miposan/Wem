...
pub fn split_block(db: &Db, id: &str, req: SplitReq) -> Result<SplitResult, AppError> {
    ...
    let result = block::run_in_transaction(&conn, || split_block_inner(&conn, id, req))?;
...
fn split_block_inner(
    ...
    let document_id = block::derive_document_id(&current);
...
pub fn merge_block(db: &Db, id: &str, req: MergeReq) -> Result<MergeResult, AppError> {
    ...
    let result = block::run_in_transaction(&conn, || merge_block_inner(&conn, id, &editor_id))?;
...
fn merge_block_inner(
    ...
    if !children.is_empty() {
        ...
        let new_document_id = block::derive_document_id_from_parent(conn, reparent_target_id)?;
