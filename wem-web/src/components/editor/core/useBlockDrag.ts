...
export function useBlockDrag(options: UseBlockDragOptions) {
  ...
  const computeDropPosition = useCallback(
    (e: React.DragEvent, targetBlockId: string): DropPosition | null => {
      ...
      if (isContainerBlock(targetBlock)) {
        if (
          relativeX > CHILD_INDENT_THRESHOLD ||
  ...
  const isValidDrop = useCallback(
    (
      ...
      if (
        position === "child" &&
        isDescendantOf(tree, targetBlockId, dragBlockId)
      ) {
        return false;
      }
  ...
  const onDrop = useCallback(
    (e: React.DragEvent, targetBlockId: string) => {
      ...
      // 折叠 heading（有子块）拖到 before/after → move-heading-tree（子树整体移动）
      // 展开的 heading 或非 heading → move-block（只移动块本身）
      const tree = getTree();
    ...
    [onAction, isCollapsed, computeDropPosition, isValidDrop, getTree],
  );
