...
impl Block {
    ...
    pub fn from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        ...
        Ok(Block {
            ...
            // JSON 字符串 → 枚举（解析失败则用安全默认值）
            block_type: serde_json::from_str(&block_type_str).unwrap_or(BlockType::Paragraph),
            content_type: ContentType::from_str(&content_type_str).unwrap_or(ContentType::Empty),
            content: row.get("content")?,
            // JSON 字符串 → HashMap（解析失败则用空 HashMap）
            properties: serde_json::from_str(&properties_str).unwrap_or_default(),
            version: row.get("version")?,
            status: BlockStatus::from_str(&status_str).unwrap_or(BlockStatus::Normal),
