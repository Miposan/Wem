...
mod tests {
    ...
    fn find_by_id_root_block() {
        ...
        let root = find_by_id(&conn, crate::db::ROOT_ID).unwrap();
    ...
    fn exists_normal_true_for_root() {
        ...
        assert!(exists_normal(&conn, crate::db::ROOT_ID).unwrap());
    }
    ...
    fn get_status_of_root_is_normal() {
        ...
        assert_eq!(get_status(&conn, crate::db::ROOT_ID).unwrap(), "normal");
    }
    ...
    fn get_version_of_root_is_one() {
        ...
        assert_eq!(get_version(&conn, crate::db::ROOT_ID).unwrap(), 1);
    }
    ...
    fn insert_and_find_block() {
        ...
        let p = make_params("test_block_000001", crate::db::ROOT_ID, "a1");
    ...
    fn insert_duplicate_id_fails() {
        ...
        let p = make_params("dup_block_0000001", crate::db::ROOT_ID, "a1");
        insert_block(&conn, &p).unwrap();

        let p2 = make_params("dup_block_0000001", crate::db::ROOT_ID, "a2");
    ...
    fn find_by_id_raw_includes_deleted() {
        ...
        let p = make_params("raw_test_blk_00001", crate::db::ROOT_ID, "a1");
        insert_block(&conn, &p).unwrap();
    ...
    fn find_deleted_returns_deleted_block() {
        ...
        let p = make_params("del_test_blk_00001", crate::db::ROOT_ID, "a1");
        insert_block(&conn, &p).unwrap();
    ...
    fn find_deleted_returns_error_for_normal() {
        ...
        let p = make_params("normal_test_00001", crate::db::ROOT_ID, "a1");
        insert_block(&conn, &p).unwrap();
    ...
    fn update_content_and_props_success() {
        ...
        let p = make_params("upd_test_blk_00001", crate::db::ROOT_ID, "a1");
        insert_block(&conn, &p).unwrap();
    ...
    fn update_content_and_props_version_conflict() {
        ...
        let p = make_params("conflict_blk_00001", crate::db::ROOT_ID, "a1");
        insert_block(&conn, &p).unwrap();
    ...
    fn update_status_changes_status() {
        ...
        let p = make_params("status_blk_000001", crate::db::ROOT_ID, "a1");
        insert_block(&conn, &p).unwrap();
    ...
    fn update_status_if_not_skips_same_status() {
        ...
        let p = make_params("skip_blk_00000001", crate::db::ROOT_ID, "a1");
        insert_block(&conn, &p).unwrap();
    ...
    fn update_parent_position_success() {
        ...
        // 创建两个文档
        let doc1 = make_doc_params("move_doc_00000001", crate::db::ROOT_ID, "a1");
        let doc2 = make_doc_params("move_doc_00000002", crate::db::ROOT_ID, "a2");
    ...
    fn get_max_position_returns_highest() {
        ...
        // 根块下已有 a0
        insert_block(&conn, &make_params("pos_blk_00000001", crate::db::ROOT_ID, "a1")).unwrap();
        insert_block(&conn, &make_params("pos_blk_00000002", crate::db::ROOT_ID, "a2")).unwrap();

        let max = get_max_position(&conn, crate::db::ROOT_ID).unwrap();
    ...
    fn get_max_position_empty_parent_returns_none() {
        ...
        // 新创建的空文档（无子块）
        insert_block(&conn, &make_doc_params("empty_doc_000001", crate::db::ROOT_ID, "a1")).unwrap();
    ...
    fn test_get_next_sibling_position() {
        ...
        insert_block(&conn, &make_params("sib_a_000000001", crate::db::ROOT_ID, "a1")).unwrap();
        insert_block(&conn, &make_params("sib_a_000000002", crate::db::ROOT_ID, "a3")).unwrap();

        let next = get_next_sibling_position(&conn, crate::db::ROOT_ID, "a1").unwrap();
        assert_eq!(next, Some("a3".to_string()));

        let none = get_next_sibling_position(&conn, crate::db::ROOT_ID, "a3").unwrap();
        assert_eq!(none, None);
    }
    ...
    fn test_get_prev_sibling_position() {
        ...
        insert_block(&conn, &make_params("sib_b_000000001", crate::db::ROOT_ID, "a1")).unwrap();
        insert_block(&conn, &make_params("sib_b_000000002", crate::db::ROOT_ID, "a3")).unwrap();

        let prev = get_prev_sibling_position(&conn, crate::db::ROOT_ID, "a3").unwrap();
        assert_eq!(prev, Some("a1".to_string()));

        // a1 的前驱是根块 a0（根块自身 parent_id=ROOT_ID）
        let root_prev = get_prev_sibling_position(&conn, crate::db::ROOT_ID, "a1").unwrap();
        assert_eq!(root_prev, Some("a0".to_string()));

        // a0 之前没有更小的了
        let none = get_prev_sibling_position(&conn, crate::db::ROOT_ID, "a0").unwrap();
        assert_eq!(none, None);
    }
    ...
    fn find_root_documents_lists_only_docs() {
        ...
        // 插入两个文档和一个段落
        insert_block(&conn, &make_doc_params("rootdoc_00000001", crate::db::ROOT_ID, "a1")).unwrap();
        insert_block(&conn, &make_doc_params("rootdoc_00000002", crate::db::ROOT_ID, "a3")).unwrap();
        insert_block(&conn, &make_params("para_00000000001", crate::db::ROOT_ID, "a2")).unwrap();
    ...
    fn find_descendants_three_levels() {
        ...
        insert_block(&conn, &make_doc_params("tree_doc_0000001", crate::db::ROOT_ID, "a1")).unwrap();
    ...
    fn find_descendants_empty_for_leaf() {
        ...
        insert_block(&conn, &make_params("leaf_blk_0000001", crate::db::ROOT_ID, "a1")).unwrap();
    ...
    fn find_children_paginated_basic() {
        ...
        insert_block(&conn, &make_params("page_a_00000001", crate::db::ROOT_ID, "a1")).unwrap();
        insert_block(&conn, &make_params("page_a_00000002", crate::db::ROOT_ID, "a3")).unwrap();
        insert_block(&conn, &make_params("page_a_00000003", crate::db::ROOT_ID, "a5")).unwrap();

        // 取前 2 个（含根块 a0）
        let page1 = find_children_paginated(&conn, crate::db::ROOT_ID, None, 2).unwrap();
        assert_eq!(page1.len(), 2);
        assert_eq!(page1[0].position, "a0"); // 根块自身
        assert_eq!(page1[1].position, "a1");

        // 从 a1 之后继续取
        let page2 = find_children_paginated(&conn, crate::db::ROOT_ID, Some("a1"), 2).unwrap();
        assert_eq!(page2.len(), 2);
        assert_eq!(page2[0].position, "a3");
        assert_eq!(page2[1].position, "a5");

        // a5 之后无更多
        let page3 = find_children_paginated(&conn, crate::db::ROOT_ID, Some("a5"), 2).unwrap();
    ...
    fn find_children_paginated_empty_parent() {
        ...
        insert_block(&conn, &make_doc_params("empty_par_00001", crate::db::ROOT_ID, "a1")).unwrap();
    ...
    fn find_descendant_ids_include_self_with_children() {
        ...
        insert_block(&conn, &make_doc_params("des_doc_00000001", crate::db::ROOT_ID, "a1")).unwrap();
    ...
    fn find_deleted_descendant_ids_finds_cascade_deleted() {
        ...
        insert_block(&conn, &make_doc_params("dd_doc_000000001", crate::db::ROOT_ID, "a1")).unwrap();
    ...
    fn check_is_descendant_true() {
        ...
        insert_block(&conn, &make_doc_params("anc_doc_00000001", crate::db::ROOT_ID, "a1")).unwrap();
    ...
    fn check_is_descendant_false() {
        ...
        insert_block(&conn, &make_doc_params("anc_doc_00000002", crate::db::ROOT_ID, "a1")).unwrap();
        insert_block(&conn, &make_params("anc_chd_00000002", "anc_doc_00000002", "a0")).unwrap();

        // ROOT 不是 doc 的后代
        assert!(!check_is_descendant(&conn, "anc_doc_00000002", crate::db::ROOT_ID).unwrap());
    }
    ...
    fn batch_update_status_if_bulk_restore() {
        ...
        for (i, id) in ids.iter().enumerate() {
            insert_block(&conn, &InsertBlockParams {
                ...
                ..make_params(id, crate::db::ROOT_ID, &format!("a{}", i + 2))
            }).unwrap();
        }
    ...
    fn get_position_of_child() {
        ...
        insert_block(&conn, &make_params("gp_blk_000000001", crate::db::ROOT_ID, "a3")).unwrap();

        let pos = get_position(&conn, "gp_blk_000000001", crate::db::ROOT_ID).unwrap();
        assert_eq!(pos, "a3");
    }
    ...
    fn get_position_wrong_parent_returns_error() {
        ...
        insert_block(&conn, &make_params("gp_blk_000000002", crate::db::ROOT_ID, "a1")).unwrap();
