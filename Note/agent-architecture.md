...
| Agent 构造 | Builder pattern，但限制行为 | Wem Session Manager 需要更细粒度的生命周期控制 |
...
- **Agent Builder 模式**：Session 创建时的配置组装（3.8 Session Manager 借鉴）
...
**Session Manager 的 Rust 数据结构**：

```rust
// agent/session.rs
pub struct SessionManager {
    sessions: DashMap<String, ActiveSession>, // 并发安全 HashMap
    db: SqlitePool,                           // 持久化
    provider_factory: Arc<dyn ProviderFactory>,
}

struct ActiveSession {
    session: Session,                       // 会话元数据 + 消息历史
    loop_handle: Option<JoinHandle<()>>,    // 正在运行的 AgentLoop task
    event_tx: broadcast::Sender<AgentEvent>, // Agent SSE 广播端
    pending_approval: Mutex<Option<PendingApproval>>, // 权限等待
...
通道 2: /api/v1/agent/sessions/{id}/events  （Agent 新增）
  → Agent 自身的状态事件（思考过程、工具调用、权限请求等）
  → 只在用户使用 Agent 功能时才有数据
```
...
GET  /api/v1/agent/sessions/{id}         获取会话状态
DELETE /api/v1/agent/sessions/{id}       销毁会话
...
# Agent 权限交互
POST /api/v1/agent/sessions/{id}/approve 批准工具调用
POST /api/v1/agent/sessions/{id}/deny    拒绝工具调用

# Agent 事件流
GET   /api/v1/agent/sessions/{id}/events SSE 事件流
```
...
- 同一批次多个 tool_calls → 可并行执行（ tokio::join! ）
